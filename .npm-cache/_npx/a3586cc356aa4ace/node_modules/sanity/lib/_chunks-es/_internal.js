...
import noop$1 from "lodash-es/noop.js";
...
const embeddingsCommand = {
  ...
  action: async (args, context) => {
    ...
    if (action === "status")
      ...
      output.print(chalk.green(`Embeddings enabled for dataset ${datasetName}.`)), projection && output.print(`Projection: ${projection}`), wait ? await waitForReady(output, client, projectId, datasetName, token) : output.print("Processing documents in the background. Use --wait to wait for completion.");
      ...
      output.print(chalk.green(`Disabled embeddings for dataset ${datasetName}.`)), output.print(chalk.yellow("Note: Existing embedding data will be removed."));
    }
  }
};
async function waitForReady(output, client, projectId, datasetName, token) {
  const spin = output.spinner("Waiting for embeddings to be ready...").start(), deadline = Date.now() + POLL_TIMEOUT_MS;
  ...
  try {
    for (; Date.now() < deadline; ) {
      ...
      if (settings.status !== "updating")
        throw spin.fail(`Unexpected status: ${settings.status}`), new Error(`Embeddings entered unexpected status: ${settings.status}`);
      spin.text = "Still processing...";
    }
    throw spin.fail("Timed out waiting for embeddings."), new Error("Timed out. Check status with: sanity dataset embeddings status");
  } catch (error) {
