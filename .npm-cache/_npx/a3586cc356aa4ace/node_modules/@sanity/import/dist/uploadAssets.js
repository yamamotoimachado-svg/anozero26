...
function setAssetReferences(assetRefMap, assetIds, options) {
    ...
    const batches = patchTasks.reduce((acc, task)=>{
        ...
        const overallSize = currentBatch.reduce((prev, add)=>prev + (add.tasks ? add.tasks.length : 0), 0);
...
function setAssetReferenceBatch(client, progress, tag, batch) {
    ...
    return retryOnFailure(()=>batch.reduce(reducePatch, client.transaction()).commit({
        ...
        }).then(progress).then(()=>batch.reduce((prev, add)=>prev + add.tasks.length, 0)));
}
...
function reducePatch(trx, documentTasks) {
    return trx.patch(documentTasks.documentId, (patch)=>{
        documentTasks.tasks.forEach((task)=>patch.setIfMissing({
                [task.path]: {
