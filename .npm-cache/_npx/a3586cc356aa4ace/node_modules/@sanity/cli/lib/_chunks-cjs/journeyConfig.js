...
function assembleJourneyIndexContent(schemas) {
  const sortedSchema = schemas.slice().sort((a, b) => a.name > b.name ? 1 : -1), imports = sortedSchema.map((schema) => `import { ${schema.name} } from './${schema.name}'`), exports$1 = sortedSchema.map((schema) => schema.name).join(","), fileContents = `${imports.join(`
`)}

export const schemaTypes = [${exports$1}]`;
